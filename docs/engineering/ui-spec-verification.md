...
| URL 状态 | `type=cloud|physical` 可恢复资源类型；切换类型时会清理不再适用的 GPU 条件，远程故障查询参数已移除 |
...
| 自动化 | 覆盖静态目录、页面、Hash 路由、卡片、状态、键盘/ARIA 与业务约束扫描 |
...
| 项目 | 结果 |
|---|---|
| 正式路由与商品读取 | 云服务器、GPU 云服务器和物理机均从商城同一内置目录按 `product` ID 恢复；缺失、类型不匹配和不可配置状态均实测通过 |
...
| 商城上下文 | 实测搜索、站点、计算类型、GPU 型号、GPU 数量和可配置筛选恢复；主滚动区从最大 `scrollTop=18` 返回后恢复为 18；当前目录不足两页，页码恢复由 `page=2` 状态单元测试覆盖 |
