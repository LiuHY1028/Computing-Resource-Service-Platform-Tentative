...
| URL 状态 | `type=cloud|physical` 可恢复资源类型；`viewState=loading|error|empty` 仅用于内部状态检查且未进入正式导航；切换类型时会清理不再适用的 GPU 条件 |
...
| Empty 与 No Result | 类型空资源与筛选无结果使用不同标题、说明和操作；空资源操作清除 `viewState`，无结果提供清除搜索与重置筛选 |
...
| 项目 | 结果 |
|---|---|
| 正式路由与商品读取 | 云服务器、GPU 云服务器和物理机均从商城同一 Repository 按 `product` ID 恢复；缺失、类型不匹配、不可配置、持续 Loading、Error/Retry 状态均实测通过 |
| 标准标题栏 | 两类页面均由 AppShell 的 `PageTitleBar` 显示“配置云服务器”或“配置物理机”；旧大型自定义导视区已移除，演示边界改为正文轻量提示 |
| 云服务器闭环 | 通用计算 C8 完成名称、HostPath/NFS 切换、可选镜像、SSH、来源、端口新增/编辑/删除、全表校验、确认和提交；镜像为空时同样成功，申请编号为 `APP-CLOUD-001` |
| 系统盘与镜像 | CPU/GPU 商品的系统盘均以静态规格显示 30 GB 存储容量且无输入控件；镜像默认 `null`，可从具体镜像切回“不选择镜像”，摘要、确认和成功状态显示“未选择（可选）” |
| GPU 镜像兼容 | 加速计算 G1 展示 GPU 规格与 GPU 运行环境镜像；CPU 商品不展示 GPU 专用镜像；两类商品均可不选择镜像 |
| 物理机差异 | 整机通用计算 P1 完成名称、用途、SSH 意向、来源、确认和提交；申请编号为 `APP-PHYSICAL-001`；页面没有系统盘、数据盘、HostPath、NFS 或镜像选择区 |
| 交付流程 | 物理机明确显示支付成功后部署和基础初始化、可用后在“我的资源”查看交付与连接信息；`OQ-015` 仅保留操作系统选择、镜像关系和重装能力，不提供未经确认的选择器 |
| 配置摘要与成功边界 | 两类摘要随表单实时更新；确认弹窗和成功页均显示“本次仅完成原型配置演示，未生成真实订单或计算资源。” |
| 校验与焦点 | 空必填项会更新 ARIA 错误并把焦点定位到首个错误；端口错误留在 Modal 内；端口 Modal 关闭后焦点返回“新增端口规则” |
| Modal 与 Portal | 1366 × 768 下端口 Modal 边界 left 403/right 963/top 110/bottom 658，完全位于视口；Select listbox 实测挂载到 `BODY` |
| 草稿与离开 | 修改后显示未确认状态并保存版本化会话草稿；返回商城出现离开确认，取消后字段保持，确认后正常返回；清除草稿恢复初始配置 |
| 商城上下文 | 实测搜索、站点、计算类型、GPU 型号、GPU 数量和可配置筛选恢复；主滚动区从最大 `scrollTop=18` 返回后恢复为 18；当前目录不足两页，页码恢复由 `page=2` Repository 单元测试覆盖 |
| 1920 × 1080 | 云服务器和物理机摘要保持 Sticky；摘要与轨道均为 `overflow-y: visible`、`max-height: none`，内容高度等于滚动高度且无横向溢出 |
| 1366 × 768 | 摘要轨道按视口高度降级为 `position: static`；摘要无内部滚动，云服务器由 Main Content 主滚动区自然滚动且无横向溢出 |
| MiSans VF | 两个目标视口的 Computed Style 均以 `MiSans VF` 为首选，Font Loading API 返回 true |
| 控制台 | 两类完整流程、全部状态、上下文恢复和两个目标视口检查后 warning/error 列表为空 |
| 视觉工作流 | 本轮只做临时浏览器 DOM、布局和交互检查；没有生成、保存、提交或在文档中引用新的页面评审截图 |
| 自动化 | `npm run test:run` 通过 28 个测试文件、173 项测试，覆盖生产化文案扫描、标准标题栏、静态系统盘、可选镜像、共享摘要、网络、校验、草稿、上下文恢复和工程约束 |

详细页面结构、暂定假设、演示状态地址和下一阶段接入边界见 `docs/engineering/purchase-configuration.md`。
