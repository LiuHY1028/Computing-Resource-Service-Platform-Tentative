...
Task 05B 已将 `BUY-01`、`BUY-02` 替换为正式购买配置原型。商城继续只负责发现、筛选和进入配置；配置、确认和演示结果的工程边界见 `purchase-configuration.md`。
...
| `src/features/marketplace/services/marketplaceNavigationContext.ts` | 保存筛选查询、页码和 AppShell 主滚动位置，供购买页返回时恢复 |
| `src/pages/PurchasePage.tsx` | Task 05B 购买配置编排入口；通过商城公开数据边界读取同一商品 |
...
当前本地 fixture 目录包含 6 个云服务器和 4 个物理机规格，展示字段使用“东部/西部算力中心”“通用/高性能加速卡 80GB”等正式中性命名。云服务器覆盖 CPU 计算、GPU 计算和 1/2 张加速卡；物理机覆盖 CPU 整机与 4/8 张加速卡整机。fixture 只提供当前前端所需规格，不建立真实库存、可用容量或商务规则。
...
筛选值仍不组成深链。进入购买页前，页面把完整查询、当前页和主滚动位置保存为版本化会话导航上下文；购买页返回时一次性恢复。直接刷新商城只由 `type` 参数恢复资源类型，其他筛选安全回退默认值。
...
| Loading | 首次查询的短暂加载态；正常筛选使用零延迟本地读取以减少布局跳动；`?viewState=loading` 可固定展示 | 筛选区和当前上下文保持可见 |
| Error | `?viewState=error` 的首次读取失败，显示明确错误 | “重新加载”产生真实重试；同一页面重试后成功 |
| Empty | `?viewState=empty` 返回当前类型目录总数为 0 | “查看另一类资源”同时退出空目录参数，展示另一类正常目录 |
| No Result | 目录本身有数据，但搜索/筛选后的结果数为 0 | 有搜索词时清除搜索；始终可重置筛选 |
| Disabled | 商品 `configurable` 为 false | 卡片仍显示；按钮禁用；附近文案和 Tooltip 解释原因 |

`viewState=loading|error|empty` 只用于内部状态检查，不出现在正式菜单、筛选控件或业务数据模型。缺失、`normal` 或非法值均按正常商城处理。默认 `/marketplace` 始终展示正常目录。
...
入口通过同一数据访问边界按 ID 回显所选规格，并校验商品类型与目标路由一致。正式购买配置原型可返回 `/marketplace?type=cloud` 或 `/marketplace?type=physical` 并恢复同一浏览会话的筛选、页码与滚动位置。不可配置商品不会从卡片触发路由，直接访问时也会被配置页阻止。
...
| 站点 | 单层“东部/西部算力中心”，不宣称未确认的地域层级 | `OQ-023` |
...
- 资源真实交付、开通、生命周期和购后管理；
- 商品独立详情路由；
- 真实数据盘、HostPath、NFS、镜像拉取、网络、软件或挂载操作；
...
Task 05B 继续复用本页商品 ID、联合类型与 `getMarketplaceProductById`，没有建立第二份规格。商城在导航前保存完整浏览上下文，购买页通过轻量 Router state 请求恢复；页面刷新至少由 URL 保留资源类型。购买页产生的草稿和 DEMO 结果不反向修改商城商品，也不写入订单或资源模块。详见 `docs/engineering/purchase-configuration.md`。
...
/marketplace?viewState=error
/marketplace?viewState=empty
```
