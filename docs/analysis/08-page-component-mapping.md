...
|---|---|---|---|
| `MarketplaceLayout` | 56px 白色产品顶栏 → 蓝色商城 Hero / 能力条 / 价格矩阵 / 完整商品目录 / 规格对比 → 共用深色页脚；仅用于 `/marketplace/*` | Task 11 的区域拆分、Task 16 指定参考页、`REQ-002/003/016` | 顶栏当前区、跨区入口、消息反馈、响应式导航；不引入控制台侧栏 |
| `SoftwareCenterLayout` | 共用产品顶栏 → 较短软件 Hero / 能力条 / 版本矩阵 / 软件目录 / 适配覆盖 → 共用深色页脚；仅用于 `/software/*` | Task 16 指定参考页、软件发现与安装目标、`REQ-017` | 搜索无结果、兼容禁用、安装处理/失败/提交反馈；同品牌但不复刻硬件价格模板 |
...
| `SoftwareAdaptationTable` | 使用当前资源兼容判断和安装关系计算兼容资源、已关联资源及覆盖率 | Task 16 软件适配、资源联动与 `REQ-017` | 零兼容资源、覆盖率、详情入口和安装后刷新；不冒充外部使用热度 |
