...
> 本文描述应用内可交互路径。订单、账单、收银台和前端主状态由 DEC-045～047 定义；真实支付、权限和基础设施履约仍不在本地应用边界内。
...
- 预付费：配置 → 确认订单 → 创建订单和账单 → 收银台 → 支付 → 开通中 → 已完成 → 资源详情。
- 按量：配置 → 确认开通 → 创建订单 → 开通中 → 已完成 → 按账期形成后付费账单。
- 续费、续租和收费扩容遵循同一预付费交易；支付完成后才更新期限或容量。
- 免费挂载、卸载和网络规则直接确认并写操作记录，不创建订单或账单。
- 每个对象只有一个当前主状态；健康、到期、容量、计费和自动续费作为辅助信息。
- 应用内支付结果不代表真实外部扣款，应用内资源结果不代表真实基础设施已交付。

### 1.3 流程索引

| 流程 ID | 流程名称 | 主入口 | 成功落点（暂定） |
|---|---|---|---|
| `FLOW-CLOUD-BUY` | 云服务器购买交易 | `MKT-01` | `CHK-01` → `ORD-02`/`RES-02` |
| `FLOW-PHYSICAL-BUY` | 物理机购买交易 | `MKT-01` | `CHK-01` → `ORD-02`/`RES-04` |
| `FLOW-STORAGE-BUY` | 存储购买交易 | `STO-03` | `CHK-01` → `ORD-02`/`STO-02` |
| `FLOW-RENEW` | 续费、续租与收费扩容 | 资源/存储详情 | `CHK-01` → 原资源详情 |
...
| `FLOW-ORDER-VIEW` | 订单与账单查看流程 | `ORD-01`/`BILL-01` | 订单、账单、收银台或关联资源 |
...
下列早期流程图保留需求演进证据；凡与本节冲突，均以本节和 DEC-045～047 为准。
