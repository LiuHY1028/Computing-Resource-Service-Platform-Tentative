...
> 本文定义产品信息结构与导航关系；交易与前端状态模型以 DEC-045～047 为准，真实支付、权限及基础设施状态映射仍不在本地应用内实现。
...
9. **订单、账单与资源分离。** 新购及收费变更形成订单；预付费同步形成账单并进入收银台，按量资源按账期形成账单；订单和账单各自只有一个主状态。（已确认；Task 15，DEC-045～047）
...
| 7 | 订单与账单 | 订单、账单、操作记录 | `ORD-01/02`、`BILL-01/02`、`CHK-01`、`OPS-01` | 查看订单与费用快照、完成支付并追踪关键操作结果 | Task 15 明确订单、账单、收银台及单一状态口径 |
...
| `STO-03` | 购买存储 | 存储管理 | 商品配置器 | 产品、规格、容量、周期与条件挂载；确认订单后进入收银台 |
| `STO-04` | 文件管理 | 存储管理 | 文件工作台 | 管理适用存储内的目录、文件和本地任务 |
...
| `ORD-01` | 订单列表 | 订单与账单 | 列表页 | 展示订单类型、商品、配置、金额、创建时间与单一主状态 |
| `ORD-02` | 订单详情 | 订单与账单 | 详情页 | 展示订单快照、时间线、关联账单与资源 |
| `BILL-01` | 账单列表 | 订单与账单 | 列表页 | 展示账单类型、关联订单、金额、账期、到期时间与单一主状态 |
| `BILL-02` | 账单详情 | 订单与账单 | 详情页 | 展示费用明细、关联订单/资源、付款与时间信息 |
| `CHK-01` | 收银台 | 订单与账单 | 交易页 | 核对订单和费用、选择支付方式、支付或取消待支付订单 |
| `OPS-01` | 操作记录 | 订单与账单 | 列表页 | 汇总资源、存储、网络、软件及订单操作结果 |
...
| 确认订单并进入收银台 | 展示连接信息、监控、挂载和操作记录入口 | 不在资源详情重新承担商品发现和购买比较 |

预付费购买创建订单与账单后进入 `CHK-01`；支付完成进入开通流程，应用内履约完成后可进入资源详情。按量购买创建订单并开通，后续按账期形成账单。真实基础设施未接入时，连接信息保持待分配。
...
| 订单 | `ORD-01/02` | 配置页、收银台、账单和资源详情 | 承载商业行为、价格快照、单一状态与履约关系 |
| 账单 | `BILL-01/02` | 订单详情、收银台和资源详情 | 承载应付金额、费用快照、账期和单一付款状态，不与订单合并 |
