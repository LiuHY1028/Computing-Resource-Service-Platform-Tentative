...
资源商城按“蓝色算力 Hero—四项能力—真实价格矩阵—完整筛选目录—GPU 规格对比—算力价值图文”组织。价格矩阵和商品卡均读取同一商品目录与统一价目，规格对比中的进度只表达当前可购买卡数规模，不表示理论性能；原创插画与文案说明从规格筛选、价格核对到购后管理的连续路径。

软件中心按“软件部署 Hero—四项能力—精选版本矩阵—完整软件目录—部署价值图文”组织。低价值的安装覆盖率表已由版本、兼容判断、费用边界和安装后管理链路说明替代；详情、目标资源选择、订单或安装任务、操作记录和资源详情联动保持一致。
